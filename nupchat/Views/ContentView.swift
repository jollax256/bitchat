...
struct ContentView: View {
    ...
    private var backgroundColor: Color {
        NupChatTheme.primaryBackground(colorScheme)
    }

    private var textColor: Color {
        NupChatTheme.primaryText(colorScheme)
    }

    private var secondaryTextColor: Color {
        NupChatTheme.secondaryText(colorScheme)
    }
...
// MARK: - Body
                ...
                ZStack {
                    ...
                    HStack {
                        AppDrawer(isPresented: $showDrawer) {
                            // Settings navigation is handled by the tab view in MainNavigationShell
                            NotificationCenter.default.post(name: NSNotification.Name("ShowSettingsTab"), object: nil)
                        }
                        .frame(width: 300)
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(alignment: .center, spacing: 8) {
                TextField(
                    ...
                    .foregroundColor(NupChatTheme.tertiaryText(colorScheme))
                )
                ...
                .background(
                    Capsule()
                        .fill(NupChatTheme.secondaryBackground(colorScheme))
                )
                .overlay(
                    Capsule()
                        .stroke(NupChatTheme.divider(colorScheme), lineWidth: 0.5)
                )
    ...
    private func handleOpenURL(_ url: URL) {
        guard url.scheme == "nupchat" else { return }
        switch url.host {
        case "user":
    ...
    private var mainHeaderView: some View {
        VStack(spacing: 8) {
            HStack(spacing: 16) {
                ...
                // Centered App/Channel Info
                VStack(spacing: 0) {
                    Text(locationManager.selectedChannel.displayName)
                        .font(.bitchatSystem(size: 17, weight: .bold))
                        .foregroundColor(textColor)
                        .onTapGesture(count: 3) {
                    ...
                    HStack(spacing: 4) {
                        if viewModel.isScanning {
                            // Spinning/pulse indicator for scanning
                            Circle()
                                .trim(from: 0, to: 0.7)
                                .stroke(Color.orange, lineWidth: 1.5)
                                .frame(width: 8, height: 8)
                                .rotationEffect(Angle(degrees: viewModel.isScanning ? 360 : 0))
                            ...
                            Circle()
                                .fill(headerOtherPeersCount > 0 ? Color.green : Color.gray)
                                .frame(width: 6, height: 6)
                            Text("\(headerOtherPeersCount) active \(headerOtherPeersCount == 1 ? "peer" : "peers")")
                                .font(.bitchatSystem(size: 11, weight: .medium))
                                .foregroundColor(headerOtherPeersCount > 0 ? Color.green : secondaryTextColor)
                        }
                    }
                    .padding(.top, 1)
                }
...
private extension ContentView {
    ...
    private var composerAccentColor: Color {
        viewModel.selectedPrivateChatPeer != nil ? Color.orange : NupChatTheme.accent
    }
    ...
    private func sendButtonView(enabled: Bool) -> some View {
        ...
        return Button(action: sendMessage) {
            ZStack {
                Circle()
                    .fill(enabled ? activeColor : Color.gray.opacity(0.3))
