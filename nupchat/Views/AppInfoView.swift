            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack(spacing: 8) {
                    Image(systemName: "exclamationmark.triangle.fill")
                        .foregroundColor(NupChatTheme.error)
                ...
                Text(Strings.Warning.message)
                    ...
                    .foregroundColor(NupChatTheme.error.opacity(0.9))
                    .fixedSize(horizontal: false, vertical: true)
            }
            .padding(16)
            .background(
                RoundedRectangle(cornerRadius: NupChatTheme.cardCornerRadius, style: .continuous)
                    .fill(NupChatTheme.error.opacity(0.1))
            )
            .overlay(
                RoundedRectangle(cornerRadius: NupChatTheme.cardCornerRadius, style: .continuous)
                    .stroke(NupChatTheme.error.opacity(0.3), lineWidth: 1)
            )
        }
        .padding()
    }
...
struct SectionHeader: View {
    ...
    var body: some View {
        Text(title)
            ...
            .foregroundColor(NupChatTheme.primaryText(colorScheme))
    }
}
