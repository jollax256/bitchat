...
struct DeliveryStatusView: View {
    ...
    private var iconColor: Color {
        switch status {
        case .sending, .sent:
            return isOutgoing ? .white.opacity(0.6) : NupChatTheme.secondaryText(colorScheme)
        case .delivered:
            return isOutgoing ? .white.opacity(0.8) : NupChatTheme.deliveredIndicator(colorScheme)
        case .read:
            return NupChatTheme.readReceipt
        case .failed:
            return NupChatTheme.error
...
#Preview {
    ...
    VStack(spacing: 16) {
        ...
        .padding()
        .background(NupChatTheme.outgoingBubble)
        .cornerRadius(12)
